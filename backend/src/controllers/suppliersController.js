...
import { validateSupplier } from "../validators/validator.js";
// CREATE (POST)
suppliersController.postSuppliers = async (req, res) => {
  try {
    ...
    const validationError = validateSupplier({name, contactPerson, phoneNumber, email, address});
    if (validationError) {
...
suppliersController.putSuppliers = async (req, res) => {
  try {
    ...
    /* // Validar lo que venga en req.body
    const validationError = validateSupplier(updates);
    if (validationError) {
      return res.status(400).json({ message: validationError }); // si hay error, corto aquí
    } */
