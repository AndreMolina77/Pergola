...
import { validateCollection } from "../validators/validator.js";
...
collectionsController.postCollections = async (req, res) => {
  try {
    ...
    const error = validateCollection({name, description, image: imageURL, isActive});
    if (error) {
...
// READ (GET ALL PUBLIC COLLECTIONS)
collectionsController.getPublicCollections = async (req, res) => {
  try {
    // Buscar colecciones publicas
    const publicCollections = await Collections.find({ isActive: true });
    // ESTADO DE OK
    res.status(200).json(publicCollections);
  } catch (error) {
    // ESTADO DE ERROR DEL SERVIDOR
    res.status(500).json({ message: "Error al obtener colecciones publicas", error: error.message });
  }
};
// READ (GET ONE BY ID)
...
collectionsController.putCollections = async (req, res) => {
  try {
    ...
    /* // Validar lo que venga en req.body
    const error = validateCollection(updates);
    if (error) {
      return res.status(400).json({ message: error }); // si hay error, corto aquí
    } */
