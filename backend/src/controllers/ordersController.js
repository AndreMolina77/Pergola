...
ordersController.postOrders = async (req, res) => {
  try {
    ...
    const validationError = validateOrder({orderCode, customer, receiver, timetable, mailingAddress, paymentMethod, status, paymentStatus, deliveryDate, items, subtotal, total})
    if (validationError) {
...
ordersController.putOrders = async (req, res) => {
  try {
    ...
    /* // Validar lo que venga en el req.body
    const validationError = validateOrder(updates)
    if (validationError) {
      return res.status(400).json({ message: validationError }) // si hay error, corto aquí
    } */
