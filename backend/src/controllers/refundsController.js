...
import { validateRefund } from "../validators/validator.js";
// POST (CREATE)
refundsController.postRefunds = async (req, res) => {
  try {
    ...
    const validationError = validateRefund({refundCode, order, customer, requestDate, reason, comments, items, status, amount, refundMethod})
    if (validationError) {
...
refundsController.putRefunds = async (req, res) => {
  try {
    ...
    if (updates.refundCode) {
      const existingRefund = await Refunds.findOne({refundCode: updates.refundCode, _id: { $ne: req.params.id } /* Excluir el documento actual */ });
    ...
    /* // Validar lo que venga en el req.body
    const validationError = validateRefund(updates)
    if (validationError) {
      return res.status(400).json({ message: validationError }) // si hay error, corto aquí
    } */
