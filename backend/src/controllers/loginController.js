...
loginController.login = async (req, res) => {
  ...
  try {
    ...
    if (userFound) {
      ...
      // RESTRICCIÓN: Los empleados solo pueden acceder desde web
      if (platform === "mobile") {
        console.log(`Intento de login ${userType} desde móvil - BLOQUEADO`)
        return res.status(403).json({ message: "Los colaboradores deben usar la versión web" })
      }
      ...
      if (platform !== "mobile" && userType === "customer") {
        ...
        return res.status(403).json({ message: "Los clientes deben usar la aplicación móvil" })
      }
    } else {
    ...
    // Verificar si la cuenta está verificada (para clientes)
    ...
    jsonwebtoken.sign({id: userFound._id, userType, email: userFound.email, name: userFound.name, lastName: userFound.lastName}, config.JWT.secret, { expiresIn: rememberMe ? "30d" : config.JWT.expiresIn }, (err, token) => {
      if(err) {
        console.log("Error generando token:", err)
        return res.status(500).json({message: "Error interno del servidor"})
      }
