...
import { validateRawMaterial } from "../validators/validator.js";
// POST (CREATE)
rawMaterialsController.postRawMaterials = async (req, res) => {
  try {
    const { correlative, name, description, type, color, tone, toneType, texture, shape, dimension,  provider, brand, presentation, quantity, piecesPerPresentation, totalPieces, piecePrice, purchaseDate, stock } = req.body;
    ...
    const validationError = validateRawMaterial({correlative, name, description, type, color, tone, toneType, texture, shape, dimension,  provider, brand, presentation, quantity, piecesPerPresentation, totalPieces, piecePrice, purchaseDate, stock})
    if (validationError) {
      return res.status(400).json({ message: validationError }); // si hay error, corto aquí
    } */
    const newRawMaterial = new RawMaterials({ correlative, name, description, type, color, tone, toneType, texture, shape, dimension, provider, brand, presentation, quantity, piecesPerPresentation, totalPieces, piecePrice, purchaseDate: new Date(purchaseDate), stock });
...
rawMaterialsController.putRawMaterials = async (req, res) => {
  try {
    ...
    /* // Validar lo que venga en el req.body
    const validationError = validateRawMaterial(updates)
    if (validationError) {
      return res.status(400).json({ message: validationError }); // si hay error, corto aquí
    } */
