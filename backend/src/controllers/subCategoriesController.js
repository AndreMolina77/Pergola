...
import { validateSubcategory } from "../validators/validator.js";
...
subcategoriesController.postSubcategories = async (req, res) => {
  try {
    ...
    const validationError = validateSubcategory({name, description, image: imageURL, isActive});
    if (validationError) {
...
// READ (GET ALL PUBLIC SUBCATEGORIES)
subcategoriesController.getPublicSubcategories = async (req, res) => {
  try {
    // Buscar subcategorías públicas
    const publicSubcategories = await Subcategories.find({ isActive: true });
    // ESTADO DE OK
    res.status(200).json(publicSubcategories);
  } catch (error) {
    // ESTADO DE ERROR DEL SERVIDOR
    res.status(500).json({ message: "Error al obtener subcategorías públicas", error: error.message });
  }
};
// READ (GET ONE BY ID)
...
subcategoriesController.putSubcategories = async (req, res) => {
  try {
    ...
    /* // Validar lo que venga en req.body
    const validationError = validateSubcategory(updates);
    if (validationError) {
      return res.status(400).json({ message: validationError }); // si hay error, corto aquí
    } */
